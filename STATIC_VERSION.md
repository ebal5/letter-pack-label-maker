...
- **データバリデーション**: Python標準の`dataclasses`（Pydanticの代替、Pyodide互換）

### 主な機能
- ✅ 単一ラベルのPDF生成（中央配置、4丁付レイアウト対応）
- ✅ CSVファイルからの一括生成（4upレイアウト、複数ページ対応）
- ✅ 設定辞書（`config_dict`）のサポート（将来的なUI設定・localStorage連携に対応）
- ✅ セクション高さの自動検証（合計122mm）

## レイアウト仕様

### ラベルサイズ
- **ラベル幅**: 100mm
- **ラベル高さ**: 122mm（実測値ベース、外枠線含む）

### セクション高さ
- **お届け先セクション**: 69mm
- **ご依頼主セクション**: 53mm
- **区切り線の太さ**: 2.5ポイント（デフォルト値、設定で変更可能）

合計: 69mm + 53mm = 122mm

**注意**: 区切り線の太さは `SectionHeightConfig.divider_line_width` で設定可能です（0より大きく10以下のポイント単位）。
...
### 機能追加
- ✅ **完了**: 複数ラベルの一括生成（CSVファイル対応）
- ✅ **完了**: config_dictサポート（将来的なUI設定機能の基盤）
- プレビュー機能（PDF.jsを使用）
- テンプレート保存機能（LocalStorage）- 既存のconfig_dict機能を活用
- UI設定編集機能（フォントサイズ、マージン、セクション高さなど）
